...
@section('content')
    ...
    <div class="box">
        <div class="box-header row">
            <div class="col col-lg-12">
                @if($button_bulk_action && ( ($button_delete && CRUDBooster::isDelete()) || $button_selected) )
                    <div class="pull-{{ trans('crudbooster.left') }}" style="padding-right: 20px">
                        <div class="selected-action" style="display:inline-block;position:relative;">
                           {{-- <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i
                            ...
                            <ul class="dropdown-menu" style="display: block; position: inherit">
                ...
                @endif
                <div class="box-tools pull-{{ trans('crudbooster.left') }}" style="margin-top: 2px">
                    @if($button_filter)
                        ...
                        @if($button_add && CRUDBooster::isCreate())
                            @if($default_field)
                                <a href="{{ CRUDBooster::mainpath('add').'?return_url='.urlencode(Request::fullUrl()).'&parent_id='.g('parent_id').'&parent_field='.$parent_field.'&'.$default_field.'='.$default_field_value }}"
                                   id='btn_add_new_data' class="btn btn-sm btn-success" title="養殖環境新規追加" style="padding-top: 8px; font-size: 14px; padding-bottom: 8px">
                                    <i class="fa fa-plus-circle"></i> {{trans('ebi.養殖環境新規追加')}}
                                </a>
                            @else
        ...
        <div class="box-body no-padding">
            @include("admin_aquacultures_table")
        </div>
    </div>
