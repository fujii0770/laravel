@foreach($addaction as $a)
    ...
    if (isset($a['showIf'])) {
        ...
        @eval("if($query) {
          echo \"<a class='btn btn-xs btn-\$color \$customClass' title='\$title' onclick='\$confirm_box' href='\$url'><i class='\$icon'></i> $label</a>&nbsp;\";
      }");
    } else {
...
@if($button_action_style == 'button_text')
    ...
    @if(($global_privilege || CRUDBooster::isRead()) && $button_detail)
    ...
    @if(($global_privilege || CRUDBooster::isUpdate()) && $button_edit)
        <a class='btn btn-xs btn-success btn-edit' title='{{trans("crudbooster.action_edit_data")}}'
           href='{{CRUDBooster::mainpath("edit/".$row->$pk)."?parent_id=".g("parent_id")."&parent_field=".$parent_field }}'>{{trans("crudbooster.action_edit_data")}}</a>
    @endif

    @if(($global_privilege || CRUDBooster::isDelete()) && $button_delete)
