<?php
$name = str_slug($form['label'], 'ebi_child');
?>
@push('bottom')
        ...
        <div class="col-sm-10 col-sm-offset-2">
            ...
            <div id='panel-form-{{$name}}' class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class='row'>
                        <div class='col-sm-10'>
                            <div class="panel panel-default">
                                <div class="panel-heading"><i class="fa fa-pencil-square-o"></i> {{$form['child_form_label']}}</div>
                                <div class="panel-body child-form-area" id="panel-form-edit-{{$name}}">
                                    @foreach($form['columns'] as $col)
								        ...
								        <button class="btn btn-primary" onclick="showModal{{$name_column}}()" type="button"><i
                                                ...
                                                @elseif($col['type']=='decimal')
                                                    <input id='{{$name_column}}' type='number' step=".01"
                                                           {{ ($col['min'])?"min='".$col['min']."'":"" }} {{ ($col['max'])?"max='$col[max]'":"" }} name='child-{{$col["name"]}}'
                                                           class='form-control {{$col['required']?"required":""}}'
                                                            {{($col['readonly']===true)?"readonly":""}}
                                                    />
								        ...
								        <button class="btn btn-primary" id="btn-upload-{{$name_column}}" onclick="showFakeUpload{{$name_column}}()"
                                                ...
                                                @elseif($col['type']=='select')
                                                    ...
                                                    <select id='{{$name_column}}' name='child-{{$col["name"]}}'
                                                            ...
                                                            {{($col['readonly']===true)?"readonly":""}}
                                                    >
                                                        <option value=''>{{$col['label']}} {{trans('crudbooster.text_prefix_option')}}</option>
                                                        <?php
                                                        if ($col['datatable']) {
                                    ...
                                    @push('bottom')
                                        <script type="text/javascript">
                                            ...
                                            function resetForm{{$name}}() {
                                                $('#panel-form-edit-{{$name}}').find("input[type=text],input[type=number],select,textarea,input[type=hidden]").val('');
                                                $('#panel-form-edit-{{$name}}').find(".select2").val('').trigger('change');
                                                $('#btn-add-table-{{$name}}').val('{{$form["child_form_add_to"]}}');
                                                if (currentRow){
                                            ...
                                            function validateForm{{$name}}() {
                                                ...
                                                $('#panel-form-{{$name}} .required').each(function () {
                                                    var v = $(this).val();
                                                    if (v == '') {
                                                        sweetAlert("{{trans('crudbooster.alert_warning')}}", "{{$form['child_form_validation_msg']}}", "warning");
                                                        is_false += 1;
                                                    }
                                                })
                                            ...
                                            function addToTable{{$name}}() {
                                                ...
                                                @foreach($form['columns'] as $c)
                                                        @if($c['type']=='select')
                                                            trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'>" + $('#{{$name.$c["name"]}} option:selected').text() +
                                                            "<input type='hidden' name='{{$name}}-{{$c['name']}}[]' value='" + $('#{{$name.$c["name"]}}').val() + "'/>" +
                                                            "</td>";
                                                        @elseif($c['type']=='radio')
                                                            trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'><span class='td-label'>" + $('.{{$name.$c["name"]}}:checked').parent('label').text() + "</span>" +
                                                            "<input type='hidden' name='{{$name}}-{{$c['name']}}[]' value='" + $('.{{$name.$c["name"]}}:checked').val() + "'/>" +
                                                            "</td>";
                                                        @elseif($c['type']=='datamodal')
                                                            trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'><span class='td-label'>" + $('#{{$name.$c["name"]}} .input-label').val() + "</span>" +
                                                            "<input type='hidden' name='{{$name}}-{{$c['name']}}[]' value='" + $('#{{$name.$c["name"]}} .input-id').val() + "'/>" +
                                                            "</td>";
                                                        @elseif($c['type']=='upload')
                                                                @if($c['upload_type']=='image')
                                                                    trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'>" +
                                                                    ...
                                                                    "</td>";
                                                                @else
                                                                    trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'><a data-label='" + $('#{{$name.$c["name"]}} .input-label').val() + "' href='{{asset('/')}}" + $('#{{$name.$c["name"]}} .input-id').val() + "'>" + $('#{{$name.$c["name"]}} .input-label').val() + "</a>" +
                                                                    "<input type='hidden' name='{{$name}}-{{$c['name']}}[]' value='" + $('#{{$name.$c["name"]}} .input-id').val() + "'/>" +
                                                                    "</td>";
                                                                @endif
                                                        @else
                                                            trRow += "<td class='{{$c['name']}} {{$c['col_hide']?'d-none':''}}'>" + $('#{{$name.$c["name"]}}').val() +
                                                            "<input type='hidden' name='{{$name}}-{{$c['name']}}[]' value='" + $('#{{$name.$c["name"]}}').val() + "'/>" +
                                                            "</td>";
                                                        @endif
                                                    @endforeach
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <i class='fa fa-table'></i> {{$form['child_table_label']}}
                        </div>
                        <div class="panel-body no-padding table-responsive" style="max-height: 400px;overflow: auto;">
                            <table id='table-{{$name}}' class='table table-striped table-bordered'>
                                ...
                                <tr>
                                    @foreach($form['columns'] as $col)
                                        <th class="{{$col['col_hide']?'d-none':''}}">{{$col['label']}}</th>
                                    @endforeach
                                ...
                                <tr>
                                    @foreach($form['columns'] as $col)
                                        <td class="{{$col['name']}} {{$col['col_hide']?'d-none':''}}">
                                            <?php
                                            if ($col['type'] == 'select') {
                                                if ($col['datatable']) {
                                                ...
                                                if ($col['dataenum']) {
                                                    if ($col['callback_php']) {
                                                        @eval("\$value = ".$form['callback_php']($d->{$col['name']}).";");
                                                        echo "<span class='td-label'>";
                                                        echo $value;
                                                        echo "</span>";
                                                    }else{
                                                ...
                                                if ($col['callback_php']) {
                                                    @eval("\$value = ".$col['callback_php']($d->{$col['name']}).";");
                                                    echo "<span class='td-label'>";
                                                    echo $value;
                                                    echo "</span>";
                                                }else{
