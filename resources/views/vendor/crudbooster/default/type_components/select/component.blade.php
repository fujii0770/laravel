<?php $default = ! empty($form['default']) ? $form['default'] : $form['label'].trans('crudbooster.text_prefix_option');?>
        ...
        <select class='form-control' id="{{$name}}" data-value='{{$value}}' {{$required}} {!!$placeholder!!} {{$readonly}} {{$disabled}} name="{{$name}}">
            ...
            if (! $form['parent_select']) {
                ...
                if (@$form['datatable']):
                    ...
                    if (@$form['filter_condition']){
                        $selects_data->whereraw($form['filter_condition']);
                    }
