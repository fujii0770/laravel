                ...
                <script>
                    $(function () {
                        $('#{{$name}}').select2({
                            ...
                            placeholder: {
                                id: '-1',
                                text: '{{$form['label']}} {{trans('crudbooster.text_prefix_option')}}'
                            },
                            allowClear: true,
                            ajax: {
                                url: '{!! $url !!}',
...
<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        <select style='width:100%' class='form-control' id="{{$name}}"
                {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} name="{{$name}}{{($form['relationship_table']||$form['multiple'])?'[]':''}}" {{ ($form['relationship_table'])?'multiple="multiple"':(($form['multiple'])?'multiple="multiple"':'') }} >
            @if($form['dataenum'])
                <option value=''>{{$form['label']}} {{trans('crudbooster.text_prefix_option')}}</option>
                <?php
            ...
            @if($form['datatable'])
                ...
                @else
                    @if($form['datatable_ajax'] == false)
                        ...
                        $result = $result->orderby($select_title, 'desc')->get();
                        foreach ($result as $r) {
                            $option_label = $r->{$select_title};
                        ...
                        ?>
                    <!--end-datatable-ajax-->
                    @elseif($form['datatable_ajax'] == true && $form['selectedValues'])
                            <?php
                            foreach ($form['selectedValues'] as $r) {
                                $option_label = $r['label'];
                                $option_value = $r['value'];
